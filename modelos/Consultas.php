...
Class Consultas
{
	...
	{
		$sql="SELECT CONCAT(DAY(MAX(fecha_hora)),'-',MONTH(MAX(fecha_hora))) as fecha, SUM(total_compra) as total FROM ingreso WHERE estado='Aceptado' GROUP BY DATE(fecha_hora) ORDER BY DATE(fecha_hora) DESC LIMIT 0,10";
		return ejecutarConsulta($sql);
	}

	public function ventasultimos_12meses()
	{
		$sql="SELECT DATE_FORMAT(MAX(fecha_hora),'%M') as fecha, SUM(total_venta) as total FROM venta WHERE estado='Aceptado' GROUP BY YEAR(fecha_hora), MONTH(fecha_hora) ORDER BY YEAR(fecha_hora) DESC, MONTH(fecha_hora) DESC LIMIT 0,12";
		return ejecutarConsulta($sql);
	}
}
