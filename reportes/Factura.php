...
function addTVAs( $igv, $total,$moneda )
{
	...
	$this->SetFont( "Arial", "", 8);
	$this->SetXY( $re, $y1+5 );
	$this->Cell( 17,4, $moneda.number_format($total-($total*$igv/($igv+100)), 0, '.', ','), '', '', 'R');
	$this->SetXY( $re, $y1+10 );
	$this->Cell( 17,4, $moneda.number_format(($total*$igv/($igv+100)), 0, '.', ','), '', '', 'R');
	$this->SetXY( $re, $y1+14.8 );
	$this->Cell( 17,4, $moneda.number_format($total, 0, '.', ','), '', '', 'R');
